...
static Bridge southbridges[] = {
	...
	{ 0x8086, 0x27bc, pIIxget, pIIxset },	/* Intel 82801BAM/CAM/DBM */
	...
	{ 0x10DE, 0x0364, nil, nil },		/* NVIDIA MCP55 LPC Bridge */
	{ 0x10DE, 0x0370, nil, nil },		/* NVIDIA MCP55 PCI Bridge */
	{ 0x1166, 0x0200, nil, nil },		/* ServerWorks ServerSet III LE */
	{ 0x1002, 0x4377, nil, nil },		/* ATI Radeon Xpress 200M */
	{ 0x1002, 0x4372, nil, nil },		/* ATI SB400 */
	{ 0x1002, 0x439D, nil, nil },		/* AMD LPC Host controller */
};
...
pcicfginit(void)
{
	...
	if(nobios) {
		...
		DBG("Sizes: mem=%llux size=%llux io=%lux\n",
		...
		pcilog("Mask sizes: mem=%llux io=%lux\n", mema, ioa);
...
pcilhinv(Pcidev* p)
{
	...
	for(t = p; t != nil; t = t->link) {
		...
		if(t->ioa.bar || t->ioa.size)
			print("ioa:%.8llux %lld ", t->ioa.bar, t->ioa.size);
		if(t->mema.bar || t->mema.size)
			print("mema:%.8llux %lld ", t->mema.bar, t->mema.size);
		if(t->bridge)
