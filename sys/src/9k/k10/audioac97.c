...
sethwp(Ctlr *ctlr, long off, void *ptr)
{
	...
	csr32w(ctlr, off+Bar, PCIWADDR64(ptr));
...
ac97reset1(Audio *adev, Ctlr *ctlr)
{
	...
	if(p->vid == 0x8086 &&
	  ...
	  ((p->mem[2].bar & 1) == 0 && (p->mem[3].bar & 1) == 0)){
		...
		if(ctlr->mmmix == nil){
			print("ac97: vmap failed for mmmix 0x%08llux\n", p->mem[2].bar & ~0xF);
			return -1;
		}
		ctlr->mmreg = vmap(p->mem[3].bar & ~0xF, p->mem[3].size);
		if(ctlr->mmreg == nil){
			print("ac97: vmap failed for mmreg 0x%08llux\n", p->mem[3].bar & ~0xF);
	...
	ctlr->micring.buf = mallocalign(Bufsize, 8, 0, 0);
	ctlr->micring.nbuf = Bufsize;
	ctlr->micring.ri = 0;
	ctlr->micring.wi = 0;

	ctlr->inring.buf = mallocalign(Bufsize, 8, 0, 0);
	ctlr->inring.nbuf = Bufsize;
	ctlr->inring.ri = 0;
	ctlr->inring.wi = 0;

	ctlr->outring.buf = mallocalign(Bufsize, 8, 0, 0);
	...
	for(i = 0; i < Ndesc; i++){
		...
		if(ctlr->sis7012)
			...
			size = Blocksize / 2;
		ctlr->micdesc[i].addr = PCIWADDR64(ctlr->micring.buf + off);
		ctlr->micdesc[i].size = Ioc | size;
		ctlr->indesc[i].addr = PCIWADDR64(ctlr->inring.buf + off);
		ctlr->indesc[i].size = Ioc | size;
		ctlr->outdesc[i].addr = PCIWADDR64(ctlr->outring.buf + off);
