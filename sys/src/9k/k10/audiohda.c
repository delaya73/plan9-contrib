...
streamalloc(Ctlr *ctlr, Stream *s, int num)
{
	...
	s->blds = mallocalign(Nblocks * sizeof(Bld), 128, 0, 0);
	if(r->buf == nil || s->blds == nil){
		print("hda: no memory for stream\n");
		return -1;
	}
	for(i=0; i<Nblocks; i++){
		pa = PCIWADDR64(r->buf) + i*Blocksize;
		s->blds[i].addrlo = pa;
...
hdamatch(Pcidev *p)
{
	while(p = pcimatch(p, 0, 0)){
		...
		case (0x8086 << 16) | 0xa0c8:	/* Intel Tiger Lake */
...
hdareset1(Audio *adev, Ctlr *ctlr)
{
	...
	if(p->mem[0].bar & 1){
		print("hda: bar0 %.8llux: not memory\n", p->mem[0].bar);
		return -1;
	}
