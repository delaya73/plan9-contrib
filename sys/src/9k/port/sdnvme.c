...
nvmerio(SDreq *r)
{
	...
	if((i = sdfakescsirw(r, &lba, &count, &rw)) != SDnostatus)
		return i;
	r->rlen = nvmebio(unit, r->lun, rw == SDwrite, r->data, count, lba);
...
setupqueues(Ctlr *ctlr)
{
	...
	/* SQID[1..nonline]: submission queue per cpu */
	for(i=1; i<=nsq; i++){
		sq = &ctlr->sq[i];
...
nvmeenable(SDev *sdev)
{
	...
	snprint(name, sizeof(name), "%s (%s)", sdev->name, sdev->ifc->name);
	ctlr->vector = intrenable(ctlr->pci->intl, nvmeintr, ctlr, ctlr->pci->tbdf, name);

	if(waserror()){
		print("%s: %s\n", name, up->errstr);
		nvmedisable(sdev);
		sdev->nunit = 0;	/* hack: prevent further probing */
		return 0;
	}
	
	pa = PCIWADDR64(cqalloc(ctlr, &ctlr->cq[0], ctlr->mpsshift));
	dmaflush(1, ctlr->cq[0].base, 1<<ctlr->mpsshift);
	ctlr->reg[ACQBase0] = pa;
	ctlr->reg[ACQBase1] = pa>>32;

	pa = PCIWADDR64(sqalloc(ctlr, &ctlr->sq[0], ctlr->mpsshift));
...
{
	return seprint(p, e, "sd%c %s %d units\n", sdev->idno, sdev->ifc->name, sdev->nunit);
}
